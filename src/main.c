...
#if defined(SGL_MATH_IMPLEMENTATION)
...
__m128 sglm_vec_sine(__m128 vec_num) {
    ...
    vec_2pi = _mm_set1_ps(3.1415f * 2);
    vec =_mm_cmpnlt_ps(vec_num, _mm_set1_ps(3.1415f));
    vec = _mm_and_ps(vec, vec_2pi);
    vec_num = _mm_sub_ps(vec_num, vec);
    vec =_mm_cmpngt_ps(vec_num, _mm_set1_ps(-3.1415f));
    vec = _mm_and_ps(vec, vec_2pi);
    vec_num = _mm_add_ps(vec_num, vec);

    vec_abs = sglm_vec_absolute(vec_num);
    vec = _mm_mul_ps(vec_abs, _mm_set1_ps(-4.0f / (3.1415f * 3.1415f)));
    vec = _mm_add_ps(vec, _mm_set1_ps(4.0f / 3.1415f));
...
float sglm_cosine(float x) {
    __m128 vec_num = _mm_set1_ps(x + 3.1415f / 2.0f);
...
void sglm_sine_cosine(float x, float *s, float *c) {
    __m128 vec_both = _mm_set_ps(0.0f, 0.0f, x + 3.1415f / 2.0f, x);
...
sglm_Mat4x4 sglm_mat4x4_set_trans_scale_rot(float x, float y, float scalex, float scaley, float angle) {
    ...
    while(angle > 360.0f) {
     ...
     do { if(!((angle >= 0.0f)) { __debugbreak(); } } while(0) && (angle <= 360.0f));
...
sglm_Mat4x4 sglm_mat4x4_mul(sglm_Mat4x4 *a, sglm_Mat4x4 *b) {
    ...
    res.e[0] = _mm_mul_ps(b->e[0],  _mm_shuffle_ps(( a->e[0]), ( a->e[0]),  ((0) | (( 0) << 2) | (( 0) << 4) | (( 0) << 6)) // Redefinition of intels __MM_SHUFFLE.));
    res.e[1] = _mm_mul_ps(b->e[0],  _mm_shuffle_ps(( a->e[1]), ( a->e[1]),  ((0) | (( 0) << 2) | (( 0) << 4) | (( 0) << 6)) // Redefinition of intels __MM_SHUFFLE.));
    res.e[2] = _mm_mul_ps(b->e[0],  _mm_shuffle_ps(( a->e[2]), ( a->e[2]),  ((0) | (( 0) << 2) | (( 0) << 4) | (( 0) << 6)) // Redefinition of intels __MM_SHUFFLE.));
    res.e[3] = _mm_mul_ps(b->e[0],  _mm_shuffle_ps(( a->e[3]), ( a->e[3]),  ((0) | (( 0) << 2) | (( 0) << 4) | (( 0) << 6)) // Redefinition of intels __MM_SHUFFLE.));

    res.e[0] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((1->e[1]) | (( 1->e[1]) << 2) | (( 1->e[1]) << 4) | (( 1->e[1]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(1)), (  SGLM_COPY_ONE_LANE_TO_ALL(1->e[1]->e[0])), res.e[0]);
    res.e[1] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((1->e[1]) | (( 1->e[1]) << 2) | (( 1->e[1]) << 4) | (( 1->e[1]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(1)), (  SGLM_COPY_ONE_LANE_TO_ALL(1->e[1]->e[1])), res.e[1]);
    res.e[2] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((1->e[1]) | (( 1->e[1]) << 2) | (( 1->e[1]) << 4) | (( 1->e[1]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(1)), (  SGLM_COPY_ONE_LANE_TO_ALL(1->e[1]->e[2])), res.e[2]);
    res.e[3] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((1->e[1]) | (( 1->e[1]) << 2) | (( 1->e[1]) << 4) | (( 1->e[1]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(1)), (  SGLM_COPY_ONE_LANE_TO_ALL(1->e[1]->e[3])), res.e[3]);

    res.e[0] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((2->e[2]) | (( 2->e[2]) << 2  ) | (( 2->e[2]) << 4) | (( 2->e[2]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(2)), (  SGLM_COPY_ONE_LANE_TO_ALL(2->e[2]->e[0])), res.e[0]);
    res.e[1] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((2->e[2]) | (( 2->e[2]) << 2) | (( 2->e[2]) << 4) | (( 2->e[2]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(2)), (  SGLM_COPY_ONE_LANE_TO_ALL(2->e[2]->e[1])), res.e[1]);
    res.e[2] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((2->e[2]) | (( 2->e[2]) << 2) | (( 2->e[2]) << 4) | (( 2->e[2]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(2)), (  SGLM_COPY_ONE_LANE_TO_ALL(2->e[2]->e[2])), res.e[2]);
    res.e[3] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((2->e[2]) | (( 2->e[2]) << 2) | (( 2->e[2]) << 4) | (( 2->e[2]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(2)), (  SGLM_COPY_ONE_LANE_TO_ALL(2->e[2]->e[3])), res.e[3]);

    res.e[0] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((3->e[3]) | (( 3->e[3]) << 2) | (( 3->e[3]) << 4) | (( 3->e[3]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(3)), (  SGLM_COPY_ONE_LANE_TO_ALL(3->e[3]->e[0])), res.e[0]);
    res.e[1] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((3->e[3]) | (( 3->e[3]) << 2) | (( 3->e[3]) << 4) | (( 3->e[3]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(3)), (  SGLM_COPY_ONE_LANE_TO_ALL(3->e[3]->e[1])), res.e[1]);
    res.e[2] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((3->e[3]) | (( 3->e[3]) << 2) | (( 3->e[3]) << 4) | (( 3->e[3]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(3)), (  SGLM_COPY_ONE_LANE_TO_ALL(3->e[3]->e[2])), res.e[2]);
    res.e[3] =  _mm_add_ps(_mm_mul_ps((  _mm_shuffle_ps((), (),  ((3->e[3]) | (( 3->e[3]) << 2) | (( 3->e[3]) << 4) | (( 3->e[3]) << 6)) // Redefinition of intels __MM_SHUFFLE.), ( SGLM_COPY_ONE_LANE_TO_ALL(3)), (  SGLM_COPY_ONE_LANE_TO_ALL(3->e[3]->e[3])), res.e[3]);

    return(res);
}

#endif // defined(SGL_MATH_IMPLEMENTATION)
